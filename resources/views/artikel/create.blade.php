        ...
        <div class="mx-8">
            <div class="bg-secondary p-8 rounded-xl">
                <form method="POST" enctype="multipart/form-data" class="flex flex-col gap-2">
                    @csrf
                    <div>
                        <label for="judul">Judul</label><input type="text" name="judul" id="judul" placeholder="Judul Artikel" class="form-input w-full">
                        <p class="text-red-400 text-sm">{{$errors->has("judul") ? $errors->first("judul") : ""}}</p>
                    </div>
                    <div>
                        <label for="deskripsi">Deskripsi</label><textarea name="deskripsi" id="deskripsi" cols="30" rows="10"></textarea>
